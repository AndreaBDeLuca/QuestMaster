...
public class QuestRemoteDataSource implements QuestDataSource {
    private final FirebaseFirestore firestore;

    @Inject
    public QuestRemoteDataSource(FirebaseFirestore firestore) {
    ...
    public Task<List<Quest>> getQuests(List<String> questIds) {
        return firestore.collection("quests").whereIn("id", questIds).get().continueWith(task -> {
            List<Quest> quests = new ArrayList<>();
            for (QueryDocumentSnapshot document : task.getResult()) {
                Quest quest = document.toObject(Quest.class);
                quest.setId(document.getId());
    ...
    public Task<List<Quest>> getQuestsByOwnerId(String ownerId) {
        return firestore.collection("quests").whereEqualTo("ownerId", ownerId).get().continueWith(task -> {
            List<Quest> quests = new ArrayList<>();
            for (QueryDocumentSnapshot document : task.getResult()) {
                Quest quest = document.toObject(Quest.class);
                quest.setId(document.getId());
